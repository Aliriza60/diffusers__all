...
if ap.single_file_or_not == 1:
    ...
    pipeline = StableDiffusionUpscalePipeline.from_single_file(ap.model_path, safety_checker=None).to("cuda")

elif ap.single_file_or_not == 2:

    pipeline = StableDiffusionUpscalePipeline.from_pretrained(ap.model_name, safety_checker=None,use_safetensors=True,torch_dtype=torch.float16).to("cuda")
